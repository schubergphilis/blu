    ...
    public class PowerShellRunspaceManager : IDisposable
    {
        ...
        private string CreateRunspace(string[] parts)
        {
            switch (parts.Length)
            {
                ...
                case 3:
                    var userData = new PowerShellRunspace.UserData(parts[1]);
        ...
        private string CreateRunspace(string runspace, PowerShellRunspace.UserData userData)
        {
            if (_runspaces.ContainsKey(runspace))
            {
                return "Exit0:Runspace " + runspace + " already exists, command ignored.";
            }
            _runspaces[runspace] = new PowerShellRunspace(userData);
